...
const CustomerList = () => {
  ...
  const editCustomer = async (e) => {
    e.preventDefault();
    if (!editingCustomer.customerName || !editingCustomer.phoneNumber) {
  ...
  const updateStatus = async (id, status) => {
    try {
      const res = await axios.patch(`${API_BASE_URL}/customers/${id}`, {
        status,
      });
  ...
  const DetailModal = () => {
    ...
    const statusActiveCls = {
      Pending:
        "bg-gradient-to-br from-amber-400 to-orange-500 border-transparent text-white shadow-lg shadow-amber-400/30",
      Completed:
        "bg-gradient-to-br from-emerald-400 to-teal-600 border-transparent text-white shadow-lg shadow-emerald-400/30",
      Delivery:
        "bg-gradient-to-br from-pink-500 to-rose-600 border-transparent text-white shadow-lg shadow-pink-500/30",
    };
    ...
    return (
      <div className="fixed inset-0 z-[200] flex items-start justify-center p-4 sm:p-6 pt-[10vh] sm:pt-[10vh]">
        ...
        <div
          className="relative z-10 w-full bg-white dark:bg-[#0f172a] rounded-[2rem] sm:rounded-3xl shadow-[0_30px_80px_-8px_rgba(0,0,0,0.5)] border border-slate-200/60 dark:border-slate-700/60 flex flex-col overflow-hidden"
          style={{
            maxWidth: "480px",
            maxHeight: "92vh",
        ...
        >
          ...
          <div
            className={`bg-gradient-to-br ${heroBg} px-5 sm:px-6 py-5 sm:py-6 flex-shrink-0 relative overflow-hidden`}
          >
            {/* Decorative blobs */}
            ...
            <div className="flex items-center justify-between mb-4 relative">
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/20 border border-white/30 text-white text-[10px] sm:text-xs font-bold uppercase tracking-wider">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  title="Export PDF"
                  className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-white/20 hover:bg-white/35 text-white flex items-center justify-center transition-all duration-200 hover:scale-110 active:scale-90"
                >
                  <FileText className="w-4 h-4" />
                </button>
                <button
                  onClick={() => setSelectedCustomer(null)}
                  className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-white/20 hover:bg-white/35 text-white flex items-center justify-center transition-all duration-300 hover:scale-110 hover:rotate-90 active:scale-90"
                >
                  <X className="w-4 h-4" />
                </button>
              </div>
            ...
            <div className="flex items-center gap-4 relative">
              <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-2xl bg-white/25 border-2 border-white/40 flex items-center justify-center shadow-xl flex-shrink-0">
              ...
              <div className="min-w-0">
                ...
                <p className="flex items-center gap-1.5 text-white/80 text-xs sm:text-sm font-medium mt-0.5">
          ...
          {/* ── Scrollable body ── */}
          <div className="overflow-y-auto flex-1 p-4 sm:p-6 space-y-4 sm:space-y-6 custom-scrollbar">
            {/* Status switcher */}
            <div className="bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-slate-200 dark:border-slate-800 p-4">
              ...
              <div className="grid grid-cols-3 gap-2">
                {[
                  { val: "Pending", Ic: Clock, label: "Wait" },
                  { val: "Completed", Ic: CheckCircle2, label: "Done" },
                  { val: "Delivery", Ic: Truck, label: "Sent" },
                ].map(({ val, Ic, label }) => {
                  const active = (selectedCustomer.status || "Pending") === val;
                  return (
                    <button
                      key={val}
                      onClick={() => updateStatus(selectedCustomer._id, val)}
                      className={`flex flex-col items-center gap-1.5 py-3 rounded-xl border text-[10px] font-bold uppercase tracking-wide transition-all duration-200 active:scale-95 ${
                        active
                          ? statusActiveCls[val]
                          : "bg-white dark:bg-slate-800 text-slate-500 dark:text-slate-400 border-slate-200 dark:border-slate-700 hover:border-pink-400 hover:text-pink-500 hover:scale-105"
                      }`}
                    >
              ...
              {[
                {
                  icon: CalendarDays,
                  label: "Date",
                  value: selectedCustomer.entryDate || "—",
                  grad: "from-blue-400 to-indigo-500",
                },
                {
                  icon: Monitor,
                  label: "Model",
                  value: selectedCustomer.modelName,
                  grad: "from-pink-500 to-rose-500",
                },
                {
                  icon: Lock,
                  label: "Pass",
                  value: selectedCustomer.devicePassword || "None",
                  grad: "from-violet-500 to-purple-600",
                },
                {
                  icon: IndianRupee,
                  label: "Amount",
                  value: `₹${selectedCustomer.approximateAmount}`,
                  grad: "from-emerald-400 to-teal-500",
                },
                {
                  icon: Star,
                  label: "Staff",
                  value: selectedCustomer.attenderName || "—",
                  grad: "from-amber-400 to-orange-500",
                },
              ].map(({ icon: Ic, label, value, grad }) => (
                <div
                  key={label}
                  className="bg-white dark:bg-slate-900/50 rounded-2xl border border-slate-100 dark:border-slate-800 p-3 hover:shadow-md hover:-translate-y-0.5 transition-all duration-200"
                >
                  <div
                    className={`w-7 h-7 rounded-lg bg-gradient-to-br ${grad} flex items-center justify-center mb-2 shadow`}
                  >
            ...
            {/* Problem */}
            <div className="bg-white dark:bg-slate-900/50 rounded-2xl border border-slate-100 dark:border-slate-800 p-4">
            ...
            <div className="flex gap-3 pt-1">
              <button
                ...
                className="flex-1 flex items-center justify-center gap-2 bg-gradient-to-r from-amber-400 to-orange-500 text-white py-3.5 rounded-2xl font-bold text-sm shadow-lg shadow-amber-400/25 transition-all hover:scale-[1.02] active:scale-95"
              >
                <Edit3 className="w-4 h-4" /> Edit
              </button>
              <button
                onClick={() => setSelectedCustomer(null)}
                className="px-5 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 py-3.5 rounded-2xl font-bold text-sm border border-slate-200 dark:border-slate-700 transition-all hover:bg-rose-500 hover:text-white hover:border-transparent active:scale-95"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
                ...
                {statusOpts.map(({ val, Ic, grad, shadow }) => {
                  ...
                  return (
                    <button
                      ...
                      key={val}
                      onClick={() =>
                        setEditingCustomer({ ...editingCustomer, status: val })
                      }
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-6 mb-8">
        <div>
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs font-black text-pink-500 uppercase tracking-[0.2em]">
              Management
            </span>
          </div>
                ...
                <div className="flex flex-wrap items-center gap-3">
                    ...
                    <div className="relative" ref={exportMenuRef}>
                        <button
                            ...
                            className="flex items-center justify-center gap-2 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 px-4 py-2.5 rounded-xl font-bold border border-slate-200 dark:border-slate-700 hover:border-pink-400 hover:text-pink-600 hover:shadow-lg hover:shadow-pink-500/10 transition-all active:scale-95 text-xs sm:text-sm h-11"
                        >
                            <Download className="w-4 h-4" /> <span className="hidden sm:inline">Export PDF</span> <ChevronDown className={`w-3.5 h-3.5 transition-transform duration-300 ${showExportMenu ? 'rotate-180' : ''}`} />
                        </button>

                        {showExportMenu && (
                            <div className="absolute left-0 sm:left-auto sm:right-0 top-full mt-2 w-64 sm:w-72 bg-white dark:bg-[#0f172a] rounded-2xl shadow-2xl shadow-slate-900/20 border border-slate-200 dark:border-slate-700 z-[100] overflow-hidden"
                                ...
                                <div className="p-3 space-y-1">
                                    ...
                                    {[
                                        ...
                                        <button key={preset}
                                            ...
                                            className="w-full text-left px-3 py-2.5 rounded-xl text-xs sm:text-sm font-semibold text-slate-700 dark:text-slate-300 hover:bg-pink-50 dark:hover:bg-pink-500/10 hover:text-pink-600 dark:hover:text-pink-400 transition-all duration-150 flex items-center gap-2"
                                        >
                                            {label}
                                        </button>
                                    ))}
                                ...
                                <div className="p-3">
                                    ...
                                    <div className="space-y-2">
                                        <div className="grid grid-cols-2 gap-2">
                                            <div>
                                                <label className="text-[9px] font-bold text-slate-500 uppercase tracking-widest block mb-1 ml-1">Start</label>
                                                <input type="date"
                                                    value={customStart}
                                                    onChange={e => setCustomStart(e.target.value)}
                                                    className="w-full px-2 py-1.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-[10px] text-slate-800 dark:text-white focus:ring-2 focus:ring-pink-500/20 focus:border-pink-500 outline-none transition-all"
                                                />
                                            </div>
                                            <div>
                                                <label className="text-[9px] font-bold text-slate-500 uppercase tracking-widest block mb-1 ml-1">End</label>
                                                <input type="date"
                                                    value={customEnd}
                                        ...
                                        <button onClick={handleCustomExport}
                                            className="w-full h-10 flex items-center justify-center gap-2 bg-gradient-to-r from-pink-600 to-rose-500 text-white rounded-xl text-[10px] font-black uppercase tracking-wider shadow-lg shadow-pink-500/25 transition-all active:scale-95">
                                            <CalendarDays className="w-3.5 h-3.5" /> Export Range
                                        </button>
                                    </div>
                                </div>
                    ...
                    <button onClick={() => fetchCustomers(true)}
                        className="flex items-center justify-center gap-2 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 px-4 py-2.5 rounded-xl font-bold border border-slate-200 dark:border-slate-700 hover:border-pink-400 hover:shadow-lg transition-all active:scale-95 text-xs sm:text-sm h-11">
                    ...
                    <div className="relative group flex-1 min-w-[140px]">
                        ...
                        <input type="text" placeholder="Search..."
                            ...
                            className="w-full bg-white dark:bg-[#0f172a] border border-slate-200 dark:border-slate-700 rounded-xl pl-11 pr-4 py-2.5 text-sm h-11 text-slate-900 dark:text-white focus:outline-none focus:ring-4 focus:ring-pink-500/10 focus:border-pink-500 transition-all shadow-sm placeholder-slate-400 dark:placeholder-slate-600" />
                    </div>

                    <div className="bg-white dark:bg-[#0f172a] border border-slate-200 dark:border-slate-700 p-1 rounded-xl flex gap-1 shadow-sm h-11">
                        {[['table', Table], ['card', LayoutGrid]].map(([mode, Icon]) => (
                            <button key={mode} onClick={() => setViewMode(mode)}
                                className={`px-2.5 rounded-lg transition-all duration-300 ${viewMode === mode ? 'bg-pink-600 text-white shadow-lg shadow-pink-500/30 scale-105' : 'text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 hover:scale-105'}`}>
      ...
      {loading ? (
        ...
        <>
                            ...
                            <thead className="bg-slate-50 dark:bg-slate-900/60 border-b border-slate-100 dark:border-slate-800">
                                <tr>
                                    {['Customer', 'Device Model', 'Problem', 'Status', 'Actions'].map(h => (
                                        <th key={h} className={`px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest ${h === 'Actions' ? 'text-right' : ''}`}>{h}</th>
                                    ))}
                                </tr>
                            </thead>
                                    ...
                                    return (
                                        <tr key={c._id} className="hover:bg-pink-500/5 group transition-all duration-200">
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-3">
                                                    <div className="w-9 h-9 rounded-xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-pink-600 font-black group-hover:bg-pink-600 group-hover:text-white group-hover:scale-110 transition-all duration-300 italic text-sm flex-shrink-0">
                                                        {c.customerName[0]}
                                                    </div>
                                                    <div className="max-w-[150px]">
                                                        <p className="font-bold text-slate-800 dark:text-white group-hover:text-pink-600 transition-colors text-sm truncate">{c.customerName}</p>
                                                        <p className="text-slate-400 text-[10px] mt-0.5">{c.phoneNumber}</p>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className="text-xs font-semibold text-slate-700 dark:text-slate-300">{c.modelName}</span>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4 text-[10px] text-slate-400 italic max-w-[160px] truncate">
                                                {c.problemDescription || 'No description'}
                                            </td>
                                            <td className="px-6 py-4">
                                                <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full border text-[10px] font-bold uppercase ${badge.cls}`}>
                                            ...
                                            <td className="px-6 py-4 text-right">
                                                <div className="flex items-center justify-end gap-1.5">
                                                    <button onClick={() => setSelectedCustomer(c)} title="View"
                                                        className="p-2 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-500 hover:bg-pink-600 hover:text-white transition-all border border-transparent shadow-sm">
                                                        <Eye className="w-3.5 h-3.5" />
                                                    </button>
                                                    <button onClick={() => setEditingCustomer(c)} title="Edit"
                                                        className="p-2 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-500 hover:bg-amber-500 hover:text-white transition-all border border-transparent shadow-sm">
                                                        <Edit3 className="w-3.5 h-3.5" />
                                                    </button>
                                                    <button onClick={() => deleteCustomer(c._id)} title="Delete"
                                                        className="p-2 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-500 hover:bg-red-500 hover:text-white transition-all border border-transparent shadow-sm">
            ...
            <span className="text-xs text-slate-400">
              ...
              of <span className="font-bold">{customers.length}</span> records
            </span>
            <span className="text-xs text-slate-400 italic">
              Pinky Mobile · Service Management
            </span>
          </div>
        </>
      ) : (
            ...
            return (
              ...
              >
                <div className="flex justify-between items-start mb-4">
                  ...
                  <span
                    className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full border text-xs font-bold uppercase ${badge.cls}`}
                  >
                    {badge.icon} {c.status || "Pending"}
                  </span>
                </div>
                <h3 className="text-base font-black text-slate-800 dark:text-white mb-1 group-hover:text-pink-600 transition-colors">
                  {c.customerName}
                </h3>
                ...
                <div className="flex-1 space-y-2 mb-4">
                  ...
                  <p className="text-xs text-slate-400 italic line-clamp-2 px-1">
